...
\\\\
Future work will involve designing a liquidation engine integrated at the protocol level, and exploring other fundamentally different AMM mechanisms (concentrated liquidity/liquidity bins for example) that create a a net improvement to the protocol.
...
To implement this efficiently on-chain without iterating over all open positions, the protocol uses a cumulative funding index. On every pool interaction (swap, open/close position), the protocol calculates the funding accrued since the last interaction and adds it to a global `unrealized_funding_fees` accumulator.

\begin{equation}
    \text{fees\_accrued} = \text{FundingRate}(r) \cdot (\sum \Delta k_i) \cdot (\text{current\_time} - \text{last\_update\_time})
\end{equation}
...
When a leverage trader closes their position, their share of the accrued funding is calculated using the cumulative index. This amount is subtracted from their final payout and is officially "realized" by adding it to the pool's base asset reserves, permanently increasing $k_{\text{current}}$.
