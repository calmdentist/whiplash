...
At its core, Whiplash uses a constant product invariant. However, to support a unified market for spot and leverage trading, it introduces the critical concepts of \textbf{real reserves} and \textbf{effective reserves}.
...
To compensate liquidity providers for the borrowed liquidity, Whiplash implements a global funding rate that continuously repays the outstanding $\Delta k$ debt, realizing it as an increase in the pool's effective reserves. This mechanism is designed to increase quadratically with total leverage, creating a strong economic incentive to maintain a healthy pool state.
...
A position becomes liquidatable when its payout is less than or equal to a liquidation buffer (e.g., 5\%) above the value required to repay its final effective debt, $\Delta k_{\text{effective}}$. Let $P$ be the calculated payout. The condition is:
\begin{equation}
P \leq \frac{\Delta k_{\text{effective}}}{R_e_{\text{payout}}} \times 1.05
\end{equation}
where $R_e_{\text{payout}}$ is the pool's effective reserve of the payout asset.

The liquidator's reward is the portion of the position's effective size ($S_{\text{effective}}$) that is not needed to restore the pool's invariant. The amount needed is $\Delta R_{\text{restore}} = \frac{\Delta k_{\text{effective}}}{R_e_{\text{collateral}}}$, where $R_e_{\text{collateral}}$ is the effective reserve of the collateral asset. The reward is:
\begin{equation}
\text{Reward} = S_{\text{effective}} - \Delta R_{\text{restore}}
\end{equation}
Upon liquidation, the pool's state is updated using the same logic as closing a position.
